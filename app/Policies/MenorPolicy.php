<?php

namespace App\Policies;

use App\Models\Menor;
use App\Models\User;
use Illuminate\Auth\Access\HandlesAuthorization;

class MenorPolicy
{
    use HandlesAuthorization;

    /**
     * Perform pre-authorization checks.
     *
     * @param  \App\Models\User  $user
     * @param  string  $ability
     * @return void|bool
     */
    public function before(User $user, $ability)
    {
        if ($user->id === 1) return true; //el 1 puede hacer cualquier cosa, pero el resto solo puede hacer aquellos métodos
        //que devuelvan un true
    }

    /**
     * Determine whether the user can view any models.
     *
     * @param  \App\Models\User  $user
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function viewAny(User $user)
    {
        return true;
    }

    /**
     * Determine whether the user can view the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Menor  $menor
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function view(User $user, Menor $menor)
    {
        return $user->id === $menor->user_id;
        //si quiero que lo pueda ver todo el mundo, le ponemos un return true; pero no podemos porque
        //la seguridad y la protección de datos no la cumpliríamos
    }

    /**
     * Determine whether the user can create models.
     *
     * @param  \App\Models\User  $user
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function create(User $user)
    {
        return $user->id === 1;
    }

    /**
     * Determine whether the user can update the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Menor  $menor
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function update(User $user, Menor $menor)
    {
        return $user->id === $menor->user_id;
    }

    /**
     * Determine whether the user can delete the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Menor  $menor
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function delete(User $user, Menor $menor)
    {
        //
    }

    /**
     * Determine whether the user can restore the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Menor  $menor
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function restore(User $user, Menor $menor)
    {
        //
    }

    /**
     * Determine whether the user can permanently delete the model.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Menor  $menor
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function forceDelete(User $user, Menor $menor)
    {
        //
    }
}
